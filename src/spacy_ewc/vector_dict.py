...
class VectorDict(Dict[str, FloatsXd]):
    ...
    def __str__(self) -> str:
        ...
        return "\n".join([f"{key}: {self._format_array(value)}" for key, value in self.items()])
    ...
    def _format_array(arr: np.ndarray, num_elements: int = 5) -> str:
        ...
        """
        # Flatten the array to 1D and select up to the first `num_elements` items.
        flat_arr = arr.ravel()[:num_elements]
        # Convert the selected portion to a list and add ellipses for truncation.
