...
from thinc.optimizers import Optimizer, Adam
from utils.extract_labels import extract_labels
from utils.generate_spacy_entities import generate_spacy_entities
...
def run_ewc_example():
    ...
    # Consolidation (EWC) example, including setting up the Fisher Information Matrix.
    ...
    original_spacy_labels = [
        ...
        ('In September 2023, SpaceX launched the Starship rocket from its Texas base, hoping to send the first humans to Mars by 2026.', {
            'entities': [(3, 17, 'DATE'), (19, 25, 'ORG'), (39, 47, 'PRODUCT'), (64, 69, 'GPE'),
                         (95, 100, 'ORDINAL'), (111, 115, 'LOC'), (119, 123, 'DATE')]})

        # Add more annotated examples as needed to expand the EWC training dataset.
    ]
    for epoch in range(2):
        nlp.update([Example.from_dict(nlp.make_doc(
            text), annotations) for text, annotations in original_spacy_labels])

    # Initialize the EWC Pipe Wrapper with the NER pipe and original labeled examples
    ...
    training_data = [
        ...
        ("John Doe and Jane Smith are colleagues at OpenAI.", {
            "entities": [(0, 8, "BUDDY"), (13, 23, "BUDDY"), (42, 48, "COMPANY")]}),
        ...
        ("Bob Lee and Carol King work for Facebook.", {
            "entities": [(0, 7, "BUDDY"), (12, 22, "BUDDY"), (32, 40, "COMPANY")]}),
    ...
    # Prepare a test sentence to evaluate the model's performance after training
    ...
    for epoch in range(10):
        ...
        # entity labels like "BUDDY" and "COMPANY" without overwriting previously learned labels, such as "PERSON" or "ORG."
        for batch in batches:
            nlp.update(examples=batch, sgd=optimizer, losses=losses)

    # Display the accumulated training loss for the NER component after all epochs
    ...
    # the original labels, demonstrating its ability to retain prior knowledge while integrating new information.
